...
function Subject() {
    ...
    const fetchSubjects = async () => {
        try {
            ...
            if (userRole === "student" || userRole === "teacher") {
                response = await axios.get(`https://e-learningg-47u8.onrender.com/subject/get-subjects-by-major/${userMajor}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            } else {
                response = await axios.get("https://e-learningg-47u8.onrender.com/subject/subjects", {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editId) {
                await axios.put(`https://e-learningg-47u8.onrender.com/subject/update-subject/${editId}`, requestData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            } else {
                await axios.post("https://e-learningg-47u8.onrender.com/subject/create-subject", requestData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
    ...
    const handleDelete = async (_id) => {
        const token = localStorage.getItem("accessToken");
        try {
            await axios.delete(`https://e-learningg-47u8.onrender.com/subject/delete-subject/${_id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
