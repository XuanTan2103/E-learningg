...
const EditClass = () => {
  ...
  const fetchClassInfo = async () => {
    ...
    try {
      ...
      const res = await axios.get(`https://e-learningg-47u8.onrender.com/class/detail-class/${classId}`, {
  ...
  useEffect(() => {
    ...
    if (classData.Major) {
      axios
        .get(`https://e-learningg-47u8.onrender.com/subject/get-subjects-by-major/${classData.Major}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => setSubjects(res.data));

      axios
        .get(`https://e-learningg-47u8.onrender.com/user/users-by-major/${classData.Major}?Role=teacher`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .then((res) => setTeachers(res.data));

      axios
        .get(`https://e-learningg-47u8.onrender.com/user/users-by-major/${classData.Major}?Role=student`, {
          // search user
          headers: { Authorization: `Bearer ${token}` },
        })
    ...
    axios.get("https://e-learningg-47u8.onrender.com/major/majors", {
      headers: { Authorization: `Bearer ${token}` },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('accessToken');
      const response = await axios.put(`https://e-learningg-47u8.onrender.com/class/update-class/${classId}`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
