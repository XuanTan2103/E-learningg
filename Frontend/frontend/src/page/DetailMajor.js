...
function DetailMajor() {
  ...
  const fetchMajor = async () => {
    try {
      const res = await axios.get(`https://e-learningg-47u8.onrender.com/major/detail-major/${id}`, {
  ...
  const fetchUserByMajor = async () => {
    try{
        const response = await axios.get(`https://e-learningg-47u8.onrender.com/user/users-by-major/${id}`, {
            headers: { Authorization: `Bearer ${token}` }
        })
  ...
  const fetchSubjectByMajor = async()=>{
    try{
        const response = await axios.get(`https://e-learningg-47u8.onrender.com/subject/get-subjects-by-major/${id}`,{
            headers: { Authorization: `Bearer ${token}` }
        })
