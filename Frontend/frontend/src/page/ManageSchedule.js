...
function ManageSchedule() {
  ...
  const fetchSchedules = useCallback(async (date) => {
    try {
      const res = await axios.get("https://e-learningg-47u8.onrender.com/schedule/get-schedule-by-day", {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        const res = await axios.get("https://e-learningg-47u8.onrender.com/class/classes", {
          headers: { Authorization: `Bearer ${token}` }
        });
        setClasses(res.data);
      } catch (error) {
  ...
  const handleSubmitSchedule = async (e) => {
    ...
    try {
      if (editingSchedule) {
        // Update lịch học
        await axios.put(
          `https://e-learningg-47u8.onrender.com/schedule/update-schedule/${editingSchedule._id}`,
          {
        ...
        // Tạo lịch học mới
        await axios.post(
          "https://e-learningg-47u8.onrender.com/schedule/create-schedule",
          {
            ClassId: selectedClass,
            Address: selectedRoom,
            Slot: selectedSlot,
  ...
  const handleDeleteSchedule = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`https://e-learningg-47u8.onrender.com/schedule/delete-schedule/${editingSchedule._id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
