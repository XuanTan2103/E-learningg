...
function ManageUser() {
    ...
    const fetchUsers = async () => {
        ...
        try {
            ...
            const response = await axios.get('https://e-learningg-47u8.onrender.com/user/users', {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this user?')) {
            try {
                await axios.delete(`https://e-learningg-47u8.onrender.com/user/delete-user/${id}`, {
                    headers: {
