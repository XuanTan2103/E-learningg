...
function Profile() {
    ...
    const fetchUserInfo = useCallback(async () => {
        try {
            ...
            const resBlogs = await axios.get(`https://e-learningg-47u8.onrender.com/blog/get-blog-by-user/${userId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDelete = async (blogId, e) => {
        ...
        try {
            const token = localStorage.getItem("accessToken");
            await axios.delete(`https://e-learningg-47u8.onrender.com/blog/delete-blog/${blogId}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
