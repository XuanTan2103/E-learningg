...
function ManageMeeting (){
  ...
  const fetchMeetings = async () => {
    ...
    try {
      let response;
      if (role === "student") {
        response = await axios.get("https://e-learningg-47u8.onrender.com/meet/meetings", {
  ...
  const handleConfirm = async (meetingId) => {
    ...
    try {
      const response = await axios.post("https://e-learningg-47u8.onrender.com/meet/respond-meeting", {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      await axios.put(
        `https://e-learningg-47u8.onrender.com/meet/update/${currentMeeting._id}`,
        {
