...
useEffect(() => {
  ...
  const fetchBlog = useCallback(async () => {
    try {
      const res = await axios.get(`https://e-learningg-47u8.onrender.com/blog/detail-blog/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setBlog(res.data);
      setLoading(false);
  ...
  const postComment = async () => {
    ...
    try {
      ...
      await axios.post(
        "https://e-learningg-47u8.onrender.com/comment/create-comment",
        {
          Content: comment,
          Blog: id,
          User: userId
  ...
  const deleteComment = async (commentId) => {
    ...
    try {
      await axios.delete(`https://e-learningg-47u8.onrender.com/comment/delete-comment/${commentId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchBlog();
  ...
  const updateComment = async (commentId) => {
    ...
    try {
      await axios.put(
        `https://e-learningg-47u8.onrender.com/comment/update-comment/${commentId}`,
